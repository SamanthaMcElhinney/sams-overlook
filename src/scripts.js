...
const bookings = fetch("http://localhost:3001/api/v1/bookings").then(
  (response) => response.json()
);

const rooms = fetch("http://localhost:3001/api/v1/rooms").then((response) =>
  response.json()
);

Promise.all([getCustomers, bookings, rooms]).then((data) => {
  console.log(data);
